...
export default function Article() {
    const { forumUser, articleData, articlePosts, recentArticles, recentPosts } = {
        forumUser: null,
        articleData: null,
        articlePosts: [],
        recentArticles: [],
    ...
    const [recentArticlesData, setRecentArticles] = useState(recentArticles || []);
    ...
    }, [articleData, recentArticles, recentPosts]);
        ...
        if (forumUser?.id) {
            try {
                ...
                if (response.errors) {
                    ...
                    router.push(`/article/${router.query.id}`);
                }
    ...
    return (
        <>
            <Meta title={`${article?.title} - Demo Foru.ms`} />
      ...
      <div className="w-full px-6">
                    <div className="lg:flex flex-wrap">
                        <div className="py-10 lg:w-2/3 w-full md:pr-6 sm:border-r border-gray-300">
                            ...
                            <h1 className="my-6 text-4xl font-medium text-gray-900">{article?.title}</h1>
                            ...
                            <p className="mt-4 text-gray-600 text-sm">{article?.body}</p>
                            {isLoading ? (
                                <div role="status">
                                ...
                                <form method="POST" onSubmit={onSubmit} className="mt-8 flex flex-col">
                                    <textarea
                                        name="body"
                                        onChange={onChange}
                                        placeholder="Post a reply to the article"
                            ...
                            {article?._count.Post > 0 && (
                                <>
                                    <div className="my-8 text-gray-900 text-xl">Posts</div>
                                    <Posts data={articlePosts} />
                                </>
                            )}
                        </div>
                        <div className="py-10 lg:w-1/3 w-full md:pl-6">
                            <h3 className="mb-5 text-gray-900 font-medium text-xl">Recent articles</h3>
                            <Articles data={recentArticlesData} limit={5} />
...
//         const articleResponse = await api.fetchArticle(id);
//         return articleResponse;
//     };
//     const articleData = await fetchArticle();
    
//     const fetchPosts = async () => {
//         const postsResponse = await api.fetchArticlePosts(id);
//         return postsResponse?.posts;
//     };
//     const articlePosts = await fetchPosts();

    
//     const fetchRecentArticles = async () => {
//         const recentArticlesResponse = await api.fetchArticles();
//         return recentArticlesResponse?.articles;
//     };
