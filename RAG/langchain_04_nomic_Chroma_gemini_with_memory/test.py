...
def create_rag_chain(vector_store, embedding_function, llm):
    ...
    def read_history(x):
        ...
        return {"context": context, "question": query, "history": formatted_history}
...
if __name__ == "__main__":
    ...
    persist_directory = os.path.join(current_dir, "db/with_memory_ConversationalRetrievalChain_tru")
    ...
    # f_context_relevance = Feedback(provider.context_relevance, name="上下文相关性")
    # f_groundedness = Feedback(
    #     provider.groundedness_measure_with_nli, name="基于上下文的准确性"
    # )
    # f_answer_relevance = Feedback(provider.pii_detection, name="回答相关性")
    # tru_chain = TruChain(
    #     rag_chain,
    #     app_id="RAG_评估",
    ...
    while True:
        ...
        if query.lower() == "q":
            ...
            eval_data["contexts"].append([response["context"]])
